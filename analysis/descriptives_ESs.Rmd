...
knitr::kable(gerson2014b_descriptives)
```
...
### Liu, S., Woo., B. M., & Spelke, E. S. (unpublished). Three-month-olds' understanding of object-directed actions that cause state changes.
```{r liu_unpublished}
liu_unpublished <- read.csv(file="formatted_data/csv/liu_unpublished.csv") %>%
  filter(trialtype=="test" & is.na(msg)) %>%
  select(subj, sex, agedays, duration.datavyu, testtrial, trialn) %>%
...
liu_unpublished_descriptives <- liu_unpublished %>% 
  group_by(MA_condition) %>%
  dplyr::summarize(x_1 = mean(unexp_look, na.rm=TRUE),
                   x_2 = mean(exp_look, na.rm=TRUE),
                   sd_1 = sd(unexp_look, na.rm=TRUE),
                   ...
                   n_babies = n(),
                   prop_girls = sum(sex=="f") / n_babies)
knitr::kable(liu_unpublished_descriptives)
```