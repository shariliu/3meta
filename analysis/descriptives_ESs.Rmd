...
```{r gerson2014b}
gerson2014b <- read.csv(file="formatted_data/csv/gerson2014a_cd.csv", header=TRUE) %>%
...
```{r gerson2014b}
gerson2014a <- read.csv(file="formatted_data/csv/gerson2014b_ibd.csv", header=TRUE) %>%
...
liu_unpublished_descriptives <- liu_unpublished %>% 
  ...
  dplyr::summarize(x_1 = mean(unexp_look, na.rm=TRUE),
                   ...
                   prop_girls = sum(sex=="f") / n_babies,
                   m_age = mean(agedays),
                  range_age = range(agedays)[2] - range(agedays)[1])
knitr::kable(liu_unpublished_descriptives, digits = 4)
```