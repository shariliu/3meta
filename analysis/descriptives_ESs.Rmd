---
title: "Summary statistics over each study"
author: "Shari Liu"
date: "10/1/2020"
...
```{r luo2011}
luo2011 <-
  read.csv(file = "formatted_data/csv_and_codebook/luo2011.csv", header =
             TRUE) %>%
  mutate(
...
luo2011_descriptives <- luo2011 %>%
  ...
  dplyr::summarize(
    ...
    r = cor(unexp_look, exp_look, method = "pearson", use =
              "pairwise.complete.obs"),
    n_babies = n(),
    prop_girls = sum(gender == "F") / n_babies
  )

knitr::kable(luo2011_descriptives)
```
...
```{r skerry2013}
skerry2013 <-
  read.csv(file = "formatted_data/csv_and_codebook/skerry2013_liu2019.csv") %>%
  filter(paper == "SCS") %>%
  mutate(
    MA_condition = case_when(
      experiment == "SCS.Exp.1" &
        training == "effective.training" ~ "1_effectiveactiontraining",
      experiment == "SCS.Exp.2" &
        training == "ineffective.training" ~ "2_ineffectiveactiontraining",
      experiment == "SCS.Exp.3" &
        training == "no.training" ~ "3_notraining",
      experiment == "SCS.Exp.4" &
        hab == "constrained" ~ "4_constrainedactionhabituation",
      experiment == "SCS.Exp.5" &
        hab == "unconstrained" ~ "5_unconstrainedactionhabituation"
    )
  ) %>%
  mutate(unexp_look_prop = prop.ineff.all,
         exp_look = avg.eff,
         unexp_look = avg.ineff)


skerry2013_descriptives <- skerry2013 %>%
  group_by(MA_condition) %>%
  dplyr::summarize(
    x_1_prop = mean(unexp_look_prop, na.rm = TRUE),
...
knitr::kable(skerry2013_descriptives)
```
...
```{r gerson2014a}
gerson2014a <-
  read.csv(file = "formatted_data/csv_and_codebook/gerson2014a_ibd.csv", header =
             TRUE) %>%
  mutate(
    experiment = case_when(
      Condition == "Active" ~ "Exp.1",
      Condition == "Observation" ~ "Exp.1",
      Condition == "Control" ~ "Exp.1"
    ),
    MA_condition = case_when(
      Condition == "Active" ~ "1_Active",
...
knitr::kable(gerson2014a_descriptives)
```
...
```{r gerson2014b}
gerson2014b <-
  read.csv(file = "formatted_data/csv_and_codebook/gerson2014b_cd.csv", header =
             TRUE) %>%
  mutate(
    experiment = case_when(
      Condition == "Act" ~ "Exp.1",
...
```{r choi2018}
choi2018 <-
  read.csv(file = "formatted_data/csv_and_codebook/choi2018.csv", header =
             TRUE) %>%
  mutate(
    experiment = case_when(
      condition == "Two-object" ~ "Exp.1",
  ...
  mutate(
    New.goal_Avg = rowMeans(cbind(New.goal_1, New.goal_2), na.rm = TRUE),
    Old.goal_Avg = rowMeans(cbind(Old.goal_1, Old.goal_2), na.rm =
                              TRUE)
  ) %>%
...
knitr::kable(choi2018_descriptives)
```
...
```{r choi_ongoing}
choi_ongoing <-
  read.csv(file = "formatted_data/csv_and_codebook/choi_ongoing.csv", header =
             TRUE) %>%
  mutate(
    experiment = case_when(
      condition == "Equidistant" ~ "Exp.1",
      condition == "Near-target" ~ "Exp.1",
      condition == "Far-target" ~ "Exp.1"
    ),
    MA_condition = case_when(
      condition == "Equidistant" ~ "1_equidistant",
...
knitr::kable(choi_ongoing_descriptives)
```
...
liu2019 <-
  ...
  mutate(
    MA_condition = case_when(
      experiment == "Exp.1" ~ "1_pickupglove",
      experiment == "Exp.2" ~ "2_pickupbarehand",
      experiment == "Exp.3" &
        hab == "constrained" ~ "3_statechangebarrier",
      experiment == "Exp.3" &
        hab == "unconstrained" ~ "3_statechangenobarrier",
      experiment == "Exp.4" ~ "4_statechangenotcausal",
      experiment == "Exp.5" &
        causal == "yes" ~ "5_statechangecausal",
      experiment == "Exp.5" &
        causal == "no" ~ "5_statechangenotcausal"
    )
  ) %>%
  mutate(unexp_look_prop = prop.ineff.all,
         exp_look = avg.eff,
...
knitr::kable(liu2019_descriptives)
```