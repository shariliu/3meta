...
We again computed Bayes Factors for each of the fixed effects, which expresses the strength of evidence for the H0 (i.e. the hypothesis that the fixed effect has no predictive power in the model) versus H1 (i.e. the hypothesis that the fixed effect has predictive power in the model). The Bayes Factor associated with each fixed effect is shown in Table X. There was anecdotal evidence that seeing an unambiguous object or location goal  (BF = `r goals.bf %>% filter(Fixed.Effect =="location_object_goal_ambiguous") %>% select(BF) %>% as.numeric()`), and seeing both objects during familiarization or habituation  (BF = `r goals.bf %>% filter(Fixed.Effect =="bothobjects_present_visible_fam") %>% select(BF) %>% as.numeric()`), influenced infants' responses at test. There was moderate evidence against the hypotheses that action training (BF = `r goals.bf %>% filter(Fixed.Effect =="training_yesno") %>% select(BF) %>% as.numeric()`), action consequence  (BF = `r goals.bf %>% filter(Fixed.Effect =="action_consequence") %>% select(BF) %>% as.numeric()`), type of agent (BF = `r goals.bf %>% filter(Fixed.Effect =="agent") %>% select(BF) %>% as.numeric()`), and infant age (BF = `r goals.bf %>% filter(Fixed.Effect =="ageday") %>% select(BF) %>% as.numeric()`), influenced infants' looking preferences.
...
When we compared the effects of different variants, we found that making the goal of the agent's actions unambiguous (marginal effect, `r report(goals.interventions,4,3,2)`), or presenting a self-propelled object (marginal effect, `r report(goals.interventions,5,3,2)`) or an entire person (`r report(goals.interventions,6,3,2)`), relative to seeing a hand appear from off-stage, increased infants' looking preference for the unexpected event. We did not find any other effects, including age, motor training, or action consequences, when taking into account all of these predictors in the same model. We did not find an effect of age, `r report(goals.interventions,8,3,2)`. These analyses included `r nrow(goals)` total infants, `r nrow(goals)-nrow(goals.cooks)` of whom were classified as influential based on Cook's Distance. Excluding these influential participants in the analysis produced similar results, except that the goal ambiguity effect crossed the threshold into significance (`r report(goals.interventions.cooks,4,3,2)`), and the two effects of the agent type crossed the threshold out of significance (marginal effect of object, `r report(goals.interventions.cooks,5,3,2)`; non-significant effect of entire person, `r report(goals.interventions.cooks,6,3,2)`).
...
library(performance)
library(see)
constraints.brms.intercept <- brm(formula = look_pref ~ 1 + (1|condition) + (1+condition|experiment) + (1+experiment|paper),
                data= constraints, 
                warmup = 100, 
                iter   = 200, 
                chains = 2, 
                ...
                save_all_pars = TRUE,
                sample_prior = TRUE)
pp_check(constraints.brms.intercept)
check_model(constraints.brms.intercept)
summary(constraints.brms.intercept)

constraints.brms.full <- brm(formula = look_pref ~ training_yesno + action_causal + action_consequence + actor_hand + agent_efficient_fam + ageday + (1|condition) + (1+condition|experiment) + (1+experiment|paper),
                data= constraints, 
                warmup = 100, 
                iter   = 200, 
                chains = 2, 
