...
(dist1 |
    dist2)  + plot_layout(widths = c(1, 4)) + plot_annotation(tag_levels = 'A')
```
...
constraints.m1 <- lmer(
  data = constraints,
  formula = look_pref ~ training_yesno + action_causal + action_consequence + actor_hand + agent_efficient_fam + ageday + (1 |
                                                                                                                             condition) + (1 | experiment) + (1 | paper)
)
...
constraints.m1.cooks.beta <- lmer(
  data = constraints.cooks,
  formula = scale(look_pref) ~ training_yesno + action_causal + action_consequence + actor_hand + agent_efficient_fam + scale(ageday) + (1 |
                                                                                                                                           condition) + (1 | experiment) + (1 | paper),
  REML = FALSE
)
...
goals.m1 <- lmer(
  data = goals,
  formula = look_pref ~ training_yesno  + action_consequence + location_object_goal_ambiguous + agent + bothobjects_present_visible_fam + ageday + (1 |
                                                                                                                                                      condition) + (1 | experiment) + (1 | paper),
...
goals.interventions <- cbind(gen.beta(effectsize::standardize(goals.m1)),
                             gen.m(goals.m1),
...
goals.m1.cooks.beta <- lmer(
  data = goals.cooks,
  formula = scale(look_pref) ~ training_yesno  + action_consequence + location_object_goal_ambiguous + agent + bothobjects_present_visible_fam + ageday + (1 |
                                                                                                                                                             condition) + (1 | experiment) + (1 | paper),
  control = lmerControl(optimizer = "Nelder_Mead")
)
