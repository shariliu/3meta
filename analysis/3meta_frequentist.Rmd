...
knitr::opts_chunk$set(comment = "#", cache = FALSE, warning = FALSE)
r = getOption("repos")
...
report <- function(table, index, places, tails, flip) {
  ...
  if (missing(flip)) {
    result <-
      paste(
        "[",
        round(table$lower[index], places),
        ",",
        round(table$upper[index], places),
        "], ß=",
        round(table$beta[index], places),
        ", B=",
        round(table$b[index], places),
        ", SE=",
        round(table$se[index], places),
        ", p",
        p,
        ", ",
        howmanytails,
...
describe <- function(dataset) {
  summary <- dataset %>%
    summarise(
      lookdiff_avg = mean(look_pref, na.rm = TRUE),
  ...
  paste(
    ...
    "Mean looking preference = ",
    round(summary$lookdiff_avg, 3),
    "seconds, ",
    "standard error (SE) = ",
    round(summary$lookdiff_SE, 3)
  )
}
```
...
```{r summary}
ind.data <- read.csv(here("preprocessed_data_for_analysis/fulldata.csv"))
str(ind.data)
ind.data.long <- ind.data %>%
  rename(expected = exp_look,
         unexpected = unexp_look) %>%
  pivot_longer(
    cols = c(expected, unexpected),
    names_to = "trial_type",
...
ind.data.summary <-
  summarySEwithin(
    ...
    betweenvars = c("task", "paper", "experiment", "condition"),
    withinvars = "trial_type"
  )

ind.pref.summary <-
  summarySE(
    data = ind.data,
    measurevar = "look_pref",
    groupvars = c("task", "paper", "experiment", "condition")
  )
...
table.goals <- ind.data %>%
  ...
  summarise(n = n(),
            agemin = floor(range(ageday)[1]),
            agemax = floor(range(ageday)[2])) %>%
  select(
    paper,
    condition,
    n,
...
```{r}
expunexp <-
  ggplot(ind.data, aes(x = log(exp_look), y = log(unexp_look))) +
  geom_point() +
...
plot2.constraints <-
  ggplot(ind.data %>% filter(task == "constraints"),
         aes(condition, look_pref)) +
  geom_boxplot(outlier.alpha = 0.2,
               position = position_dodge2(preserve = "single")) +
  geom_hline(yintercept = 0) +
...
bottomrow <-
  (plot2.goals |
     plot2.constraints) + plot_layout(widths = c(1.4, 1))
bottomrow
...
constraints$actor_hand <-
  factor(constraints$actor_hand, levels = c("bare", "gloved", "mittened")) # for summed contr first level is dropped
constraints$action_consequence <-
  relevel(as.factor(constraints$action_consequence), ref = "state_change")
constraints$agent_efficient_fam <-
  relevel(as.factor(constraints$agent_efficient_fam), ref = "yes")
constraints$training_yesno <-
  relevel(as.factor(constraints$training_yesno), ref = "yes")
constraints$action_causal <-
  relevel(as.factor(constraints$action_causal), ref = "yes")
...
constraints.b1 <- lmer(data = constraints.baseline.data,
                       formula = look_pref ~ 1 + scale(ageday) + (1|condition))
...
constraints.b1.table <- sjPlot::tab_model(
  constraints.b1,
  show.std	= TRUE,
  show.stat = TRUE,
  show.df = TRUE
...
constraints.b1.beta <-
  summary(effectsize::standardize(constraints.b1))
...
dotplot_diag(
  ...
  cutoff = "internal",
  name = "cooks.distance",
  index = constraints.baseline.data$subj
) + ylab("Cook's distance") + xlab("subjID")

excluded.subs <- c("90", "553", "86")

constraints.b1.cooks <- lmer(
...
For standard versions of the constraints task, we found no differential looking between the expected and unexpected events, `r describe(constraints.baseline.data)`, `r report(constraints.baseline,1,3,2)`. This result held when we removed 3 influential observations, identified using Cook's distance, `r report(constraints.baseline.cooks,1,3,2)`.
...
constraints.m1.table <- sjPlot::tab_model(
  constraints.m1,
  show.std	= TRUE,
  show.stat = TRUE,
  show.df = TRUE
...
constraints.where.influential <-
  full_join(constraints.summary, constraints.cooksn) %>%
  mutate(n_excluded = n - n_cooks)

knitr::kable(constraints.where.influential)

constraints.m1.cooks <- lmer(
...
constraints.m1.cooks.beta <- lmer(
  data = constraints.cooks,
...
regressionplot1 <- sjPlot::plot_model(
  constraints.m1.cooks.beta,
  type = "est",
  colors = "bw",
  sort.est = TRUE,
...
constraints.interventions.cooks <- cbind(
  ...
  gen.ci(constraints.m1.cooks)[5:12, ]
)
...
constraints.b1.cooks.cohensd <- lme.dscore(constraints.m1.cooks.beta,
           data = constraints.cooks,
           type = "lme4") %>% select(d) %>% as.data.frame()
...
```{r}
constraints.full.cooks <- lmer(
  data = constraints.cooks,
  formula = look_pref ~ training_yesno + action_causal + action_consequence + actor_hand + agent_efficient_fam + ageday + (1|condition) + (1|experiment) + (1|paper),
  REML = FALSE,
  control = lmerControl(optimizer = "Nelder_Mead", optCtrl = list(maxfun =
            200000))
...
for (predictor in constraints.predictors) {
  modelform <-
    update(constraints.full.cooks, as.formula(paste0(". ~ . -", predictor)))
  BF <- exp((BIC(modelform) - BIC(constraints.full.cooks)) / 2)
  whichrow <-
    which(constraints.bf.cooks$Fixed.Effect == as.character(predictor))
...
for (predictor in constraints.predictors) {
  modelform <-
    update(constraints.full, as.formula(paste0(". ~ . -", predictor)))
  BF <- exp((BIC(modelform) - BIC(constraints.full)) / 2)
  whichrow <-
    which(constraints.bf.full$Fixed.Effect == as.character(predictor))
...
# combine BF tables
constraints.bf <-
  full_join(constraints.bf.full, constraints.bf.cooks) %>%
  arrange(desc(BF))
```
...
goals$action_consequence <-
  relevel(as.factor(goals$action_consequence), ref = "none")
goals$agent <-
   # factor(goals$agent, levels = c("hand", "person", "animate")) # last level gets dropped in model estimates
  factor(goals$agent, levels = c("animate", "person", "hand")) # last level gets dropped in model estimates
goals$training_yesno <-
  relevel(as.factor(goals$training_yesno), ref = "yes")
goals$bothobjects_present_visible_fam <-
  relevel(as.factor(goals$bothobjects_present_visible_fam), ref = "yes")
...
goals.baseline <- cbind(gen.beta(effectsize::standardize(goals.b1)),
                        gen.m(goals.b1),
                        gen.ci(goals.b1)[3:4, ])
...
dotplot_diag(
  ...
  cutoff = "internal",
  name = "cooks.distance",
  index = goals.baseline.data$subj
) + ylab("Cook's distance") + xlab("subjID")

excluded.subs <- c("2", "7", "6", "11")

goals.b1.cooks <- lmer(
  data = goals.baseline.data %>%
    filter(!subj %in% excluded.subs),
  formula = look_pref ~ 1 + scale(ageday) + (1|condition)
)
...
For standard versions of the goals task, we no differential looking between the expected and unexpected events, `r describe(goals.baseline.data)`, `r report(goals.baseline,1,3,2)`. This held after removing 4 influential observations (Cook's Distance), `r report(goals.baseline.cooks,1,3,2)`. 
...
goals.interventions <- cbind(gen.beta(effectsize::standardize(goals.m1)), gen.m(goals.m1), gen.ci(goals.m1)[5:12, ])
...
goals.where.influential <-
  full_join(goals.summary, goals.cooksn) %>%
  mutate(n_excluded  = n - n_cooks)
...
goals.m1.cooks.beta <- lmer(
  data = goals.cooks,
  formula = scale(look_pref) ~ training_yesno  + action_consequence + location_object_goal_ambiguous + agent + bothobjects_present_visible_fam + scale(ageday) + (1 | condition) + (1 | experiment) + (1 | paper),
  control = lmerControl(optimizer = "Nelder_Mead")
)
...
regressionplot2 <- sjPlot::plot_model(
  goals.m1.cooks.beta,
  type = "est",
  colors = "bw",
  sort.est = TRUE,
...
```{r}
(regressionplot2 | regressionplot1) + plot_annotation(tag_levels = 'A')
```
...
for (predictor in goals.predictors) {
  modelform <-
    update(goals.full, as.formula(paste0(". ~ . -", predictor)))
  BF <- exp((BIC(modelform) - BIC(goals.full)) / 2)
  whichrow <-
    which(goals.bf.full$Fixed.Effect == as.character(predictor))
...
for (predictor in goals.predictors) {
  modelform <-
    update(goals.full.cooks, as.formula(paste0(". ~ . -", predictor)))
  BF <- exp((BIC(modelform) - BIC(goals.full.cooks)) / 2)
  whichrow <-
    which(goals.bf.cooks$Fixed.Effect == as.character(predictor))
