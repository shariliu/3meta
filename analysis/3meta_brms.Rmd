...
#### Trying peakier prior
```{r}
# not run, uncomment to reproduce results
constraints.brms.intercept.smallervar <- brm(formula = look_pref ~ 1 + ageday + (1|condition) + (1+condition|experiment) + (1+condition|paper),
                data= constraints,
                warmup = 10,
                iter   = 1000,
...
prior_summary(constraints.brms.intercept)


constraints.brms.intercept <- readRDS("constraints.brms.intercept.rds")
# shinystan::launch_shinystan(constraints.brms.intercept)
summary(constraints.brms.intercept)
pp_check(constraints.brms.intercept.smallervar, ndraws=100)
```
