...
gerson2014a_descriptives <- gerson2014a %>%
  ...
  dplyr::summarize(x_1 = mean(unexp_look, na.rm=TRUE),
                   ...
                   prop_girls = sum(Sex=="Female") / n_babies,
                    m_age= mean(Age*30.44),
                   range_age = range(Age*30.44)[2] - range(Age*30.44)[1])
...
gerson2014b_descriptives <- gerson2014b %>%
  ...
  dplyr::summarize(x_1 = mean(unexp_look, na.rm=TRUE),
                   ...
                   n_babies = n(),
                   prop_girls = sum(Sex=="f") / n_babies,
                   m_age= mean(Age_m*30.44),
                   range_age = range(Age_m*30.44)[2] - range(Age_m*30.44)[1])
...
```{r liu_unpublished}
liu_unpublished <- read.csv(file="formatted_data/csv_and_codebook/liu_unpublished.csv") %>%
